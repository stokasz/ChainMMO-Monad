...
describe("infra stress", () => {
  it("processes a large queued workload with worker concurrency", async () => {
    const totalActions = 5_000;
    const repository = new InMemoryQueueRepository(totalActions);
    const metrics = new ActionMetrics();

    const engine = {
      execute: vi.fn(async (action: AgentActionInput) => {
        // Tiny async yield to emulate network latency under load.
        await Promise.resolve();
        return {
          code: `${action.type.toUpperCase()}_OK`,
          txHashes: [`0x${(action.characterId ?? 0).toString(16)}`],
          deltaEvents: []
        };
      })
    };
    ...
    await waitFor(() => repository.succeeded === totalActions, 10_000);

    worker.stop();
    await runner;

    expect(repository.failed).toBe(0);
    expect(repository.succeeded).toBe(totalActions);
    expect(repository.duplicateClaims).toBe(0);

    const snapshot = metrics.snapshot();
    expect(snapshot.totals.succeeded).toBe(totalActions);
  });
});
